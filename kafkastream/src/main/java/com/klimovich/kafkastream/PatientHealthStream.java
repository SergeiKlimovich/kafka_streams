...
public class PatientHealthStream {
    ...
    public KStream<String, PatientHealth> kStream(StreamsBuilder streamsBuilder,
                                                  ...
                                                  JsonSerde<PatientHealth> patientHealthJsonSerde) {
        ...
        var patientHealthJoiner = new PatientHealthJoiner();


        var joinWindow = JoinWindows.ofTimeDifferenceAndGrace(Duration.ofSeconds(60), Duration.ofSeconds(10));

        var patientHealthJoinedStream = highPulseStream
                .join(highBodyTempStream,
                        patientHealthJoiner,
                        joinWindow,
                        StreamJoined.with(stringSerde, heartRateJsonSerde, bodyTempJsonSerde)
                                .withName("patient-health-join").withStoreName("patient-health-join-store")
                );

        patientHealthJoinedStream.peek((key, value) -> logger.info("key[{}] value[{}]", key, value))
                .to(appConfig.getPatientHealthSinkTopic(), Produced.with(stringSerde, patientHealthJsonSerde));

        logger.info("topology: {}", streamsBuilder.build().describe());

        return patientHealthJoinedStream;
    }
    ...
    @Bean
    public JsonSerde<PatientHealth> patientHealthJsonSerde() {
